...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.hitbox_rect = self.base_enemy_image.get_rect(center=self.rect.center)
    ...
    def is_within_playable_area(self, position):
        ...
        left_square = pygame.Rect(25, 100, 280, 455)
        hallway = pygame.Rect(310, 220, 180, 70)
        right_square = pygame.Rect(460, 100, 280, 450)
    ...
    # DRAW WAVE TEXT
    def draw_wave_text(self, wave):

        font = pygame.font.Font(None, 30)
        wave_text = font.render('Wave: ', 1, (255, 255, 255))
        wave_number = font.render(str(wave), True, (255, 255, 255))
        text_rect = wave_text.get_rect(center=((screen.get_width()) - 130, 15))
        number_rect = wave_number.get_rect(center=((screen.get_width()) - 90, 15))
        screen.blit(wave_text, text_rect)
        screen.blit(wave_number, number_rect)
...
wave = 1
wave_length = 1  # NUMBER OF ENEMIES IN WAVE
enemy = Enemy()
...
while running:
    ...
    for bullet in bullet_group.sprites():
        ...
        if enemy.rect.colliderect(bullet.rect):  #COLLISION BULLET WITH ENEMY KILLS ENEMY IF HEALTH 0
            enemy.health -= 20
            enemy.hit()
        if len(enemy_group) == 0:  # CHECKS IF LIST OF ENEMIES EMPTY THEN ADDS AN ENEMY FOR NEXT WAVE
            wave_length += 1
            wave += 1
